...
#if OS_TASK_CREATE_EN > 0u                      /* 创建一个任务                                   */
                     ...
                     INT8U    prio)                     //任务的优先级
{
    ...
    if (OSTCBPrioTbl[prio] == (OS_TCB *)0)   /* 确认优先级未被使用                                     */
    {
        ...
        err = OS_TCBInit(prio, psp, (OS_STK *)0, 0u, 0u, (void *)0, 0u);  /*任务控制块初始化           */
        if (err == OS_ERR_NONE)
        {
        ...
        {
            OS_ENTER_CRITICAL();
            OSTCBPrioTbl[prio] = (OS_TCB *)0;/* 放弃执行任务                                           */
            OS_EXIT_CRITICAL();
        }
        return (err);
    }
...
INT8U  OSTaskCreateExt (void   (*task)(void *p_arg), /* 创建一个任务（扩展功能）                       */
                        void    *p_arg,
                        OS_STK  *ptos,
                        INT8U    prio,
                        INT16U   id,
